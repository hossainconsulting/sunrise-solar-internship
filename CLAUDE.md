...
Engagement workspace for the **SunRise Solar Solutions Pty Ltd** simulation, the
**Administrator** certification track. Hemayet plays the incoming admin; the company
and every record in it are fictional. See `README.md` for the brief and scope.

## The org

Target org alias **`sunrise`**, a Developer Edition org authorised locally.

```bash
sf org display --target-org sunrise
sf data query --target-org sunrise --query "SELECT COUNT() FROM Account"
```
...
- `scripts/` holds the per-ticket Apex and Python fix-up scripts (`cf-NN-*`).
- `deliverables/` is the product of the engagement. `deliverables/build-log.md` records
  every change with its date, reason and the requirement it traces to, and
  `deliverables/carry-forward-tickets.md` is the open ticket list.
...
- **`.mcp.json`** starts the Salesforce DX MCP server pinned to the **`sunrise`**
  ...
  `sf org login web --alias sunrise` first.
